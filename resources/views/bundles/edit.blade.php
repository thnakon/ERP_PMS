...
@section('title', __('bundles.edit_bundle'))
@section('page-title')
    <div class="welcome-container">
        ...
        <span>{{ __('bundles.edit_bundle') }}</span>
    </div>
@endsection
...
@section('content')
    <form action="{{ route('bundles.update', $bundle) }}" method="POST" enctype="multipart/form-data" class="space-y-6"
        id="bundleForm">
        @csrf
        @method('PUT')
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="card-ios p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="name_th" value="{{ old('name_th', $bundle->name_th) }}"
                    ...
                    <div class="mt-4">
                        ...
                        <textarea name="description" rows="3" class="input-ios w-full">{{ old('description', $bundle->description) }}</textarea>
                    </div>
                </div>
                ...
                <div class="card-ios p-6">
                    ...
                    <div id="bundleProducts" class="space-y-3">
                        @foreach ($bundle->products as $index => $bundleProduct)
                            <div class="flex gap-3 items-start p-3 bg-gray-50 rounded-xl product-row">
                                <div class="flex-1">
                                    <select name="products[{{ $index }}][id]" required class="input-ios w-full"
                                        onchange="updatePricing()">
                                        <option value="">-- เลือกสินค้า --</option>
                                        @foreach ($products as $p)
                                            <option value="{{ $p->id }}" data-price="{{ $p->unit_price }}"
                                                {{ $bundleProduct->id == $p->id ? 'selected' : '' }}>
                                ...
                                <div class="w-24">
                                    <input type="number" name="products[{{ $index }}][quantity]"
                                        value="{{ $bundleProduct->pivot->quantity }}" min="1" required
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="bundle_price" id="bundlePrice"
                                    value="{{ old('bundle_price', $bundle->bundle_price) }}" step="0.01" min="0"
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="datetime-local" name="start_date"
                                value="{{ old('start_date', $bundle->start_date ? $bundle->start_date->format('Y-m-d\TH:i') : '') }}"
                                class="input-ios w-full">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="end_date"
                                value="{{ old('end_date', $bundle->end_date ? $bundle->end_date->format('Y-m-d\TH:i') : '') }}"
                                class="input-ios w-full">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="stock_limit"
                                value="{{ old('stock_limit', $bundle->stock_limit) }}" min="1"
                                class="input-ios w-full" placeholder="ไม่จำกัด">
                            @if ($bundle->sold_count > 0)
                                <p class="text-xs text-gray-500 mt-1">ขายไปแล้ว: {{ $bundle->sold_count }} ชุด</p>
                            @endif
                        </div>
                    </div>
                </div>
...
@push('scripts')
    <script>
        ...
        let productCount = {{ $bundle->products->count() }};
        ...
        function updatePricing() {
            ...
            document.querySelectorAll('.product-row').forEach(row => {
                ...
                if (option && option.dataset.price) {
                    originalPrice += parseFloat(option.dataset.price) * parseInt(qtyInput.value || 0);
                }
            });
