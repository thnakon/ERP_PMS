...
@section('title', __('promotions.edit_promotion'))
@section('page-title')
    <div class="welcome-container">
        ...
        <span>{{ __('promotions.edit_promotion') }}</span>
    </div>
@endsection
...
@section('content')
    <form action="{{ route('promotions.update', $promotion) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="card-ios p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="name_th" value="{{ old('name_th', $promotion->name_th) }}"
                        ...
                        <div>
                            ...
                            <select name="type" id="promotionType" required class="input-ios w-full"
                                ...
                                @foreach ($types as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('type', $promotion->type) === $key ? 'selected' : '' }}>{{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <textarea name="description" rows="3" class="input-ios w-full">{{ old('description', $promotion->description) }}</textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('promotions.description') }}
                                (TH)</label>
                            <textarea name="description_th" rows="3" class="input-ios w-full">{{ old('description_th', $promotion->description_th) }}</textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div id="discountValueField">
                            ...
                            <div class="relative">
                                <input type="number" name="discount_value"
                                    ...
                                    min="0" required class="input-ios w-full pr-10">
                        ...
                        <div id="buyQuantityField" class="hidden">
                            ...
                            <input type="number" name="buy_quantity"
                                value="{{ old('buy_quantity', $promotion->buy_quantity ?? 2) }}" min="1"
                                class="input-ios w-full">
                        </div>
                        ...
                        <div id="getQuantityField" class="hidden">
                            ...
                            <input type="number" name="get_quantity"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="min_purchase"
                                    value="{{ old('min_purchase', $promotion->min_purchase) }}" step="0.01"
                                    min="0" class="input-ios w-full pl-8">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="max_discount"
                                    value="{{ old('max_discount', $promotion->max_discount) }}" step="0.01"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="time" name="start_time"
                                value="{{ old('start_time', $promotion->start_time) }}" class="input-ios w-full">
                        </div>

                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 mb-1">{{ __('promotions.end_time') }}</label>
                            <input type="time" name="end_time" value="{{ old('end_time', $promotion->end_time) }}"
                                class="input-ios w-full">
                        </div>
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @php
                                ...
                                $activeDays = is_array($promotion->active_days) ? $promotion->active_days : [];
                            @endphp
                            @foreach ($days as $dayNum => $dayName)
                                <label
                                    ...
                                    <input type="checkbox" name="active_days[]" value="{{ $dayNum }}"
                                        ...
                                        {{ in_array($dayNum, old('active_days', $activeDays)) ? 'checked' : '' }}>
                ...
                <div class="card-ios p-6">
                    ...
                    @php
                        ...
                        $isAllProducts = empty($selectedProductIds) && empty($selectedCategoryIds);
                    @endphp
                    ...
                    <div class="mb-4">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" id="allProductsCheck"
                                class="rounded border-gray-300 text-ios-blue focus:ring-ios-blue"
                                onchange="toggleProductSelection(this)" {{ $isAllProducts ? 'checked' : '' }}>
                            <span class="text-sm font-medium text-gray-700">{{ __('promotions.all_products') }}</span>
                        </label>
                    </div>

                    <div id="productSelection" style="display: {{ $isAllProducts ? 'none' : 'block' }}">
                        <div class="mb-4">
                            <label
                                    ...
                                    @foreach ($products as $product)
                                        <label
                                            ...
                                            <input type="checkbox" name="products[]" value="{{ $product->id }}"
                                                class="rounded border-gray-300 text-ios-blue focus:ring-ios-blue"
                                                {{ in_array($product->id, old('products', $selectedProductIds)) ? 'checked' : '' }}>
            ...
            <div class="space-y-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            @if ($promotion->usage_count > 0)
                                <p class="text-xs text-gray-500 mt-1">ใช้ไปแล้ว: {{ $promotion->usage_count }} ครั้ง</p>
                            @endif
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="per_customer_limit"
                    ...
                    <div class="space-y-4">
                            ...
                            <select name="member_tier_id" class="input-ios w-full">
                                ...
                                @foreach ($tiers as $tier)
                                    <option value="{{ $tier->id }}"
                                        {{ old('member_tier_id', $promotion->member_tier_id) == $tier->id ? 'selected' : '' }}>
                        ...
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="new_customers_only" value="1"
                                class="rounded border-gray-300 text-ios-blue focus:ring-ios-blue"
                                {{ old('new_customers_only', $promotion->new_customers_only) ? 'checked' : '' }}>
                ...
                <button type="submit"
                    ...
                    {{ __('promotions.update_promotion') }}
                </button>
            </div>
        </div>
    </form>
