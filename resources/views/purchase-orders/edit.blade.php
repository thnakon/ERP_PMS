...
@section('title', __('po.edit') . ': ' . $order->po_number)
@section('page-title')
    <div class="flex flex-col">
        ...
        <span>{{ __('po.edit') }}</span>
    </div>
@endsection

@section('content')
    <div class="space-y-6">
        <form id="po-form" method="POST" action="{{ route('purchase-orders.update', $order) }}">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="expected_date" id="expected-date" class="form-input"
                            value="{{ $order->expected_date?->format('Y-m-d') }}">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button type="button" onclick="POForm.addItem()" class="btn-primary-sm flex items-center gap-1">
                ...
                <div id="no-items" class="text-center py-8 text-gray-400" style="display: none;">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6">
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        ...
                        <textarea name="notes" class="form-input min-h-[100px]" placeholder="{{ __('po.notes') }}...">{{ $order->notes }}</textarea>
                    </div>
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="font-semibold" id="vat">฿{{ number_format($order->vat_amount, 2) }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            ...
                            <input type="number" name="discount_amount" id="discount-input"
                                class="w-32 text-right form-input py-1" value="{{ $order->discount_amount }}"
                                min="0" step="0.01" onchange="POForm.calculateTotals()">
                        </div>
                        <div class="border-t border-gray-200 pt-3 flex justify-between">
                            <span class="font-bold text-gray-900">{{ __('po.grand_total') }}</span>
                            <span class="text-2xl font-black text-ios-blue"
                                id="grand-total">฿{{ number_format($order->grand_total, 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <tr class="item-row border-b border-gray-100" data-index="${itemIndex}">
                        <td class="py-3">
                            <select name="items[${itemIndex}][product_id]" class="form-input text-sm product-select" required onchange="POForm.onProductChange(this, ${itemIndex})">
                                ...
                                ${products.map(p => `<option value="${p.id}" data-cost="${p.cost_price}" ${data && data.product_id == p.id ? 'selected' : ''}>${p.name} (${p.sku})</option>`).join('')}
                            </select>
                        </td>
                        <td class="py-3">
                            <input type="number" name="items[${itemIndex}][ordered_qty]" class="form-input text-sm text-center qty-input" 
                                value="${data ? data.ordered_qty : 1}" min="0.01" step="0.01" required onchange="POForm.calculateRow(${itemIndex})">
                        </td>
                        <td class="py-3">
                            <input type="number" name="items[${itemIndex}][unit_cost]" class="form-input text-sm text-right cost-input" 
                                value="${data ? data.unit_cost : 0}" min="0" step="0.01" required onchange="POForm.calculateRow(${itemIndex})">
                        </td>
                        <td class="py-3">
                            <input type="number" name="items[${itemIndex}][discount_percent]" class="form-input text-sm text-center discount-input" 
                                value="${data ? data.discount_percent : 0}" min="0" max="100" step="0.01" onchange="POForm.calculateRow(${itemIndex})">
                        </td>
                        <td class="py-3 text-right">
                            <span class="font-semibold line-total">฿${data ? (data.line_total).toLocaleString() : '0.00'}</span>
                        </td>
                        <td class="py-3">
