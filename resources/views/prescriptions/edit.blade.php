...
@section('title', __('prescriptions.edit_title'))
@section('page-title')
    <div class="welcome-container">
        ...
        <span>{{ __('prescriptions.edit_title') }} - {{ $prescription->prescription_number }}</span>
    </div>
@endsection
...
@section('content')
    <form action="{{ route('prescriptions.update', $prescription) }}" method="POST" id="prescription-form">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="doctor_name"
                                value="{{ old('doctor_name', $prescription->doctor_name) }}" required class="form-input"
                                placeholder="{{ __('prescriptions.doctor_name') }}">
                        </div>
                        <div>
                            <label class="form-label">{{ __('prescriptions.doctor_license') }}</label>
                            <input type="text" name="doctor_license_no"
                                value="{{ old('doctor_license_no', $prescription->doctor_license_no) }}" class="form-input"
                                placeholder="{{ __('prescriptions.doctor_license') }}">
                        </div>
                        <div>
                            <label class="form-label">{{ __('prescriptions.hospital_clinic') }}</label>
                            <input type="text" name="hospital_clinic"
                                value="{{ old('hospital_clinic', $prescription->hospital_clinic) }}" class="form-input"
                                placeholder="{{ __('prescriptions.hospital_clinic') }}">
                        </div>
                        <div>
                            <label class="form-label">{{ __('prescriptions.doctor_phone') }}</label>
                            <input type="text" name="doctor_phone"
                                value="{{ old('doctor_phone', $prescription->doctor_phone) }}" class="form-input"
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="date" name="expiry_date"
                                value="{{ old('expiry_date', $prescription->expiry_date?->format('Y-m-d')) }}"
                                class="form-input">
                        ...
                        <div>
                            ...
                            <select name="refill_allowed" class="form-input">
                                <option value="0" {{ $prescription->refill_allowed == 0 ? 'selected' : '' }}>0
                                </option>
                                @for ($i = 1; $i <= 12; $i++)
                                    <option value="{{ $i }}"
                                        {{ old('refill_allowed', $prescription->refill_allowed) == $i ? 'selected' : '' }}>
                                        {{ $i }}
                                    </option>
                                @endfor
                            </select>
                        ...
                        <div>
                            ...
                            <textarea name="diagnosis" class="form-input min-h-[80px]" placeholder="{{ __('prescriptions.diagnosis') }}">{{ old('diagnosis', $prescription->diagnosis) }}</textarea>
                        </div>
                        <div>
                            <label class="form-label">{{ __('prescriptions.notes') }}</label>
                            <textarea name="notes" class="form-input min-h-[60px]" placeholder="{{ __('prescriptions.notes') }}">{{ old('notes', $prescription->notes) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white/80 backdrop-blur-md rounded-2xl p-6 border border-white shadow-sm">
                    ...
                    <div id="no-medications" class="hidden text-center py-8 text-gray-400">
                ...
                <div class="flex gap-3 justify-end">
                    ...
                    <button type="submit" data-no-loading
                        ...
                        {{ __('prescriptions.update_prescription') }}
                    </button>
                </div>
            </div>
        </div>
...
@push('scripts')
    <script>
        ...
        function addMedicationRow(prefill = null) {
            const container = document.getElementById('medications-container');
            const template = document.getElementById('medication-row-template');
            let html = template.innerHTML.replace(/__INDEX__/g, medicationIndex);
            ...
            if (prefill) {
                ...
                const routeSelect = row.querySelector('select[name*="[route]"]');
                const instructionsInput = row.querySelector('input[name*="[instructions]"]');

                if (productSelect) productSelect.value = prefill.product_id;
                if (quantityInput) quantityInput.value = prefill.quantity;
                if (dosageInput) dosageInput.value = prefill.dosage;
                if (frequencySelect) {
                    for (let option of frequencySelect.options) {
                        if (option.value === prefill.frequency) {
                ...
                if (routeSelect) {
                    for (let option of routeSelect.options) {
                        if (option.value === prefill.route) {
                            option.selected = true;
                            break;
                        }
                    }
                }
                if (instructionsInput) instructionsInput.value = prefill.instructions || '';
            }
