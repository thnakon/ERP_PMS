...
@section('title', __('tiers.create_title'))
@section('page-title')
    <div class="welcome-container">
        ...
        <span>{{ __('tiers.create_title') }}</span>
    </div>
@endsection
...
@section('content')
    <form action="{{ route('member-tiers.store') }}" method="POST" class="max-w-2xl mx-auto">
        @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="min_spending" value="{{ old('min_spending', 0) }}" step="0.01"
                                min="0" required class="input-ios w-full pl-8">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="discount_percent" value="{{ old('discount_percent', 0) }}"
                                step="0.01" min="0" max="100" required class="input-ios w-full pr-8">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="points_multiplier" value="{{ old('points_multiplier', 1) }}"
                                min="1" max="10" class="input-ios w-full pr-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <select name="icon" class="input-ios w-full">
                            ...
                            <option value="ph-medal" {{ old('icon') === 'ph-medal' ? 'selected' : '' }}>🏅 Medal</option>
                            <option value="ph-crown" {{ old('icon') === 'ph-crown' ? 'selected' : '' }}>👑 Crown</option>
                            <option value="ph-crown-simple" {{ old('icon') === 'ph-crown-simple' ? 'selected' : '' }}>💎
                                Crown Simple</option>
                            <option value="ph-star" {{ old('icon') === 'ph-star' ? 'selected' : '' }}>⭐ Star</option>
                            <option value="ph-star-four" {{ old('icon') === 'ph-star-four' ? 'selected' : '' }}>✨ Star Four
                            </option>
                            <option value="ph-diamond" {{ old('icon') === 'ph-diamond' ? 'selected' : '' }}>💠 Diamond
                            </option>
                            <option value="ph-trophy" {{ old('icon') === 'ph-trophy' ? 'selected' : '' }}>🏆 Trophy
                            </option>
                        </select>
                    </div>
